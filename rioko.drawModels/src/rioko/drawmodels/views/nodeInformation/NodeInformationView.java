...
public class NodeInformationView extends ZestEditorDependingViewPart implements ISelectionChangedListener{
	...
	private static SimpleNodeInformationDataLine[] getSimpleInput(DiagramNode node) {
		...
		//Aquí necesitamos usar que son nodos XMI
		if(node instanceof XMIDiagramNode) {
			XMIDiagramNode xmiNode = (XMIDiagramNode)node;
			lines.add(new SimpleNodeInformationDataLine("ID", ""+xmiNode.getId()));
			lines.add(new SimpleNodeInformationDataLine("EClass", xmiNode.getEClass().getName()));
